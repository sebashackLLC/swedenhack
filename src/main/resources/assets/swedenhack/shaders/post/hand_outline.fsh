...
// ── Shared fill-effect helpers (same logic as chams_default_v.fsh) ────────────
...
void main() {
    ...
    for (int y = -LineWidth; y <= LineWidth; y++) {
        ...
        if (s.a > maxA) { maxA = s.a; col = s.rgb; }
    }
